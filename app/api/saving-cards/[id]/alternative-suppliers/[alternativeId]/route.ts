...
import { deleteAlternativeSupplier, updateAlternativeSupplier, WorkflowError } from "@/lib/data";
...
export async function PUT(
  ...
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to update alternative supplier." },
      { status: 400 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    const user = await requireUser({ redirectTo: null });
    const { id, alternativeId } = await params;
    await deleteAlternativeSupplier(alternativeId, user.organizationId, id);
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to delete alternative supplier." },
      { status: 400 }
    );
  }
}

...
- Review of the canonical workflow: Idea, Validated, Realized, Achieved, and Canceled.
...
- End-of-pilot recap with decision summary, blockers, rollout recommendation, and export handoff.
...
- Keep the public homepage CTA as `Sign in` and `View product` until a real `Request demo` or `Start pilot` handoff route exists.

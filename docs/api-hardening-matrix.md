...
| `/api/saving-cards/[id]` | `PUT`, `PATCH`, `POST` | Yes | Action-dependent | Yes | Active organization only | Yes | Yes | `PUT`/`POST`: Yes | No | Strong | `PUT` updates, `POST` handles approve / finance-lock actions, `PATCH` intentionally rejects direct phase mutation. |
...
| `/api/phase-change-request` | `POST` | Yes | No | Yes | Active organization saving card only | Yes | Yes | No | No | Good | Workflow service enforces tenant scope and writes workflow audit trail. |
| `/api/approve-phase-change` | `POST` | Yes | Yes | Yes | Active organization request only | Yes | Yes | No | No | Good | Approval permission checks are enforced in workflow service logic. |
